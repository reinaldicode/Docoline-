...
// PERBAIKAN CRITICAL: Sesuaikan dengan tipe data VARCHAR untuk tgl_approve
// ============================================================================
if (isset($_POST['update_status']) && $_POST['update_status'] == '1' && isset($_POST['action']) && $_POST['action'] == 'finish') {
    ...
    if ($res_before) {
        while ($row = mysqli_fetch_assoc($res_before)) {
            $debug_info .= "<!-- ID: {$row['id']}, NRP: {$row['nrp']}, Status: {$row['status']}, Tgl: {$row['tgl_approve']} -->\n";
        }
    }
    echo $debug_info;
    ...
    // PERBAIKAN: Karena tgl_approve adalah VARCHAR, gunakan '-' bukan NULL
    // Karena reason adalah TEXT, gunakan empty string atau '-'
    ...
    echo "<!-- Query yang akan dieksekusi: $reset_approvers -->\n";
    
    $result_reset = mysqli_query($link, $reset_approvers);
    
    if (!$result_reset) {
        // Jika masih error, coba cara alternatif
        echo "<!-- Error dengan query pertama: " . mysqli_error($link) . " -->\n";
        
        // Cara alternatif: Update satu per satu
        ...
        if ($res_pending_ids && mysqli_num_rows($res_pending_ids) > 0) {
            while ($row_id = mysqli_fetch_assoc($res_pending_ids)) {
                ...
                if (mysqli_query($link, $update_single)) {
                    $success_count++;
                    echo "<!-- Success updating ID: $id -->\n";
                } else {
                    $error_count++;
                    echo "<!-- Error updating ID: $id - " . mysqli_error($link) . " -->\n";
                }
            }
        }
    ...
    if ($res_after) {
        while ($row = mysqli_fetch_assoc($res_after)) {
            $debug_after .= "<!-- ID: {$row['id']}, NRP: {$row['nrp']}, Status: {$row['status']}, Tgl: {$row['tgl_approve']} -->\n";
        }
    }
    echo $debug_after;
    ...
    // Cek approver yang masih Approved
